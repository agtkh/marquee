...
static short get_or_create_color_pair(short fg_color, short bg_color) {
    static short pair_cache[8][8];
    ...
    if (fg_color < 0 || fg_color > 7 || bg_color < -1 || bg_color > 7) return 0;

    short bg_index = (bg_color == -1) ? 0 : bg_color;
    if (pair_cache[fg_color][bg_index] == 0) {
        if (next_pair_id < COLOR_PAIRS) {
            init_pair(next_pair_id, fg_color, bg_color);
            pair_cache[fg_color][bg_index] = next_pair_id++;
        } else {
            return 0;
        }
    }
    return pair_cache[fg_color][bg_index];
}
...
static void parse_line(const char* line_text, MarqueeLine* line) {
    ...
    while (*p != '\0') {
        if (*p == '\x1b' && p[1] == '[') {
            ...
            if (*seq_end == 'm') {
                int code = atoi(p + 2);
                if (code == 0) { current_fg = COLOR_WHITE; current_bg = -1; }
                else if (code >= 30 && code <= 37) { current_fg = code - 30; }
